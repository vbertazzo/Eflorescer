<template>
	<section class="p-8 my-6 bg-primary-light shadow-md">
		...
		>
			<h2 class="text-lg text-center text-secondary">Login</h2>
			<div class="mt-4 flex flex-col space-y-2">
				<div>
					<label for="email" class="text-secondary">Email:</label>
					<input
						class="mt-1 py-1 px-2 text-secondary border-2 w-full border-gray-200 focus:border-primary"
						type="email"
						id="email"
						v-model.trim="email"
					/>
				...
				<div>
					...
					<input
						class="mt-1 py-1 px-2 border-2 text-secondary w-full border-gray-200 focus:border-primary"
						type="password"
						id="password"
						v-model="password"
					/>
			...
			<div class="flex space-x-4">
				<base-button type="submit" class="mt-8" :disabled="loading">
					<svg
						v-if="loading"
					...
					{{ !loading ? 'Entrar' : 'Entrando...' }}</base-button
				>
				<base-button class="mt-8" :disabled="loading" link to="/registrar"
